...
BEGIN
    DECLARE person VARCHAR(20);
    CALL  find_subject(person);
    SELECT tweetid AS 'Tweet_ID ' , tweet_content AS 'Content ' , timestamp_t AS 'Time ' 
    FROM tweet
...
BEGIN
    DECLARE person VARCHAR(20);
    CALL  find_subject(person);
    SELECT tweet.tweetid,tweet.type ,tweet.tweet_content, t.tweet_content AS refrence_content,t.username AS refrence_username, tweet.timestamp_t
